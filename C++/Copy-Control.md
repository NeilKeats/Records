...
第一个参数必须是**自身类型的引用**，任何额外参数都有默认值，虽然也可以接受非const引用的拷贝构造函数，但都建议是const。
...
定义**拷贝**或**移动**构造函数的时候，通常使用对应**基类构造函数**初始化对象的基类部分
...
class D: public Base{
public:
	D(const D& d): Base(d){ /* do something*/ }
	D(D &&d): Base(std::move(d)){ /* do something*/ }
}
...
在析构函数体执行后，对象成员会被隐式销毁，只负责销毁派生类自己分配的资源。

销毁顺序与创建顺序相反：i.派生类析构函数执行，ii.基类析构函数，以此类推
