...
* **指针**：指向该节点的各个儿子（子树），`P1`,`P2`,`P3`,...,`PM`
* **内部信息**：分别代表子树`P2`_,_`P3`_,...,_`PM`中发现的**最小关键字**的值，`k1`_,_`k2`_,_`k3`_,...,_`k{M-1}`。（第一个子树由于在最左侧，没有比它更小的子树，因此不需要存储它的最小关键字值）
...
* Insert和delete可能还需要`O(M)`的工作量来更新调整节点上的所有信息，因此最坏情况为

，不过一次Find只花费`O(logN)` 时间
